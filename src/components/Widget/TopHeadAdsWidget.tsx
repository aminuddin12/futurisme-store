...
export default function TopHeadAdsWidget() {
  ...
  useEffect(() => {
    const loadTimer = setTimeout(() => setIsLoaded(true), 600); // Sedikit lebih cepat dari slider
  ...
  if (!isLoaded) {
      return (
        <div className="w-full h-full bg-gray-100 dark:bg-gray-800 rounded-xl overflow-hidden relative">
            <div className="absolute inset-0 animate-pulse bg-gradient-to-r from-gray-100 via-gray-200 to-gray-100 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 background-animate" />
        </div>
      )
  }
  ...
  return (
    <div className="relative w-full h-full overflow-hidden rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 group cursor-pointer">
      <AnimatePresence mode='wait'>
        ...
        >
          <div className="z-10 flex-1 pr-2">
            ...
            <div className="mt-2 inline-flex items-center gap-1 text-[10px] font-bold bg-white dark:bg-gray-800 px-2 py-1 rounded-full shadow-sm text-gray-700 dark:text-gray-300 group-hover:scale-105 transition-transform">
              Cek Sekarang <Icon icon="fluent:arrow-right-16-regular" />
            </div>
          </div>
          
          <div className={`w-12 h-12 md:w-16 md:h-16 bg-white/50 dark:bg-white/10 rounded-full flex items-center justify-center ${ads[index].iconColor} shadow-sm`}>
             <Icon icon={ads[index].icon} className="text-2xl md:text-4xl transform group-hover:rotate-12 transition-transform duration-300" />
          </div>
        </motion.div>
      </AnimatePresence>
      ...
      <motion.div 
        ...
        className="absolute bottom-0 left-0 h-1 bg-primary/30 z-20"
      />
    </div>
  );
}